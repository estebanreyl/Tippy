...
class MainActivity : AppCompatActivity() {
    ...
    private fun updateSplitValues() {
        ...
        val splitNum = splitSeekBar.progress + 1

        tvSplitTipAmount.text = "%.2f".format(tipAmount/splitNum)
        tvTotalSplitAmount.text = "%.2f".format(totalAmount/splitNum)

        if (splitNum == 1){
            tvSplit.text = "$splitNum person"
        }else{
            tvSplit.text = "$splitNum people"
        }
    }
